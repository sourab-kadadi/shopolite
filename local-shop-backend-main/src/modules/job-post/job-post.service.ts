...
import { CreateCatalogDto } from '../catalog/catalog.dto';

@Injectable()
export class JobPostService {
  constructor(@InjectModel('JobPosts') private Module: Model<JobPost>, private utilsService: UtilsService, private awsService: AwsService, private catalogService: CatalogService) {}
  ...
  async updateModule(JobPostId: string, JobPostDto: JobPostUpdateDto): Promise<IModuleRes> {
    try {
      let result = await this.Module.update({ _id: JobPostId }, { $set: JobPostDto });
  ...
  async uplaodUrlToFile() {
    try {
      ...
      for(let job of getJobsOfLogoUrl) {
        let jobData: any = job;
        console.log(jobData);
        console.log(jobData.name, jobData.image_link_front, jobData.subCategoryId, jobData.giverType, jobData._id);
        try {
      let fileUplaod = await (await this.utilsService.getDataFromUrl(jobData.image_link_front)).toPromise();
      console.log(fileUplaod.data);
      let udid = uuid();
      let file:any = {
        content: fileUplaod.data,
        fileName: "catalog/" + udid,
        contentType: fileUplaod.headers["content-type"],
        ACL: 'public-read',
      }
      let awsUpload = await this.awsService.fileUploadS3(file);
      ...
      let  createCatalogDto: any = {
        categoryId:mongoose.Types.ObjectId(jobData.categoryId),
        categoryName: jobData.categoryName,
        subCategoryId:mongoose.Types.ObjectId(jobData.subCategoryId),
        subCategoryName: jobData.product_category_1stlevel,
        name: jobData.name,
        ...
        unitValue: jobData.unitValue,
        unit: jobData.unit,
        sellingPrice: 0,
      };
      try {
      await this.catalogService.createModule(createCatalogDto)
      } catch (error) {
        if (error.status == 409) {
      ...
      console.log(awsUpload);
      await this.Module.update({_id: jobData._id}, {$set: {logo: awsUpload.key, upload: "s3"}});
    } catch (error) {
      await this.Module.update({_id: jobData._id}, {$set: { upload: "not"}});
    }
    }
    } catch (error) {
      console.log(error);
