...
export class CategoryService {
  ...
  async findAllDropDownModule(match?: any): Promise<ICategoryDropDownRes> {
    try {
      let aggregate: any[] = [];
      if (match) {
        aggregate.push({$match: match});
      }
      aggregate.push({$sort: {manualRanking: 1} });
      aggregate.push({$project: {name: 1, image: 1} });
  ...
  async lookUpSubCategoryDBFind(): Promise<ICategoryDropDownRes> {
    try {
      ...
      {$project: {"name": 1, "image.filePath": 1, "subCategory._id": 1, "subCategory.name": 1, "subCategory.image.filePath": 1}}
    ]);
      if (result) {
        return {
          status: true,
  ...
  async findCategoryByIds(categoryId: any[]): Promise<ICategoryDropDownRes> {
    try {
      let result = await this.Module.aggregate([{$match: {_id: { $in: categoryId}}}, {$project: {name: 1, image: 1} }]);
      if (result) {
        return {
          status: true,
          message: ICategoryMessage.foundSuccess,
